...
async def identify_user(telegram_id: int) -> dict:
    """Checks if Telegram user is registered and has active GitHub OAuth."""
...
async def get_login_link(telegram_id: int, chat_id: int) -> str:
    """Generates the unique login/OAuth URL for the Telegram user."""
...
async def select_repo(telegram_id: int, repo: str) -> bool:
    """Sets active repository for the Telegram user."""
    data = await _post("/api/bot/select_repo", {"telegram_id": telegram_id, "repo": repo})
    return data.get("status") == "success"

async def generate_plan(telegram_id: int, chat_id: int, raw_idea: str, draft_id: Optional[int] = None, previous_markdown: Optional[str] = None, feedback: Optional[str] = None) -> dict:
    """Invokes LLM plan generation."""
    payload = {
...
async def list_drafts(telegram_id: int, chat_id: int) -> List[dict]:
    """Lists drafts for the Telegram user and chat."""
...
async def create_task(telegram_id: int, chat_id: int, task_title: Optional[str] = None) -> dict:
    """Creates a single task or bulk-creates tasks parsed from the latest draft."""
...
async def get_board(telegram_id: int) -> dict:
    """Retrieves Kanban Board columns for the user's active repository."""
...
async def claim_card(telegram_id: int, username: str, issue_number: int, repo: str) -> str:
    """Attempts to lock/claim a task card."""
